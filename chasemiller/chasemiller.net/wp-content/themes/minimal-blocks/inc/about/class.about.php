 ...
 * @since 1.0.0
 */
if ( ! class_exists( 'Minimal_Blocks_About' ) ) {
	/**
	 * Singleton class used for generating the about page of the theme.
	 */
	class Minimal_Blocks_About {
		/**
		 ...
		 *
		 * @var Minimal_Blocks_About $instance The  TI_About_Page instance.
		 */
		...
		public static function init( $config ) {
			if ( ! isset( self::$instance ) && ! ( self::$instance instanceof Minimal_Blocks_About ) ) {
				self::$instance = new Minimal_Blocks_About;
    ...
    return add_query_arg(
		...
		public function useful_plugins() {
			...
			if ( ! empty( $useful_plugins ) ) {
				if ( ! empty( $useful_plugins['content'] ) && is_array( $useful_plugins['content'] ) ) {
					...
					echo '<div class="feature-section recommended-plugins demo-import-boxed" id="plugin-filter">';
		...
		public function free_pro() {
			...
			if ( ! empty( $free_pro ) ) {
				if ( ! empty( $free_pro['free_theme_name'] ) && ! empty( $free_pro['pro_theme_name'] ) && ! empty( $free_pro['features'] ) && is_array( $free_pro['features'] ) ) {
					...
					echo '<tr><td style="border-top:0;"></td><td style="border-top:0;"></td><td style="border-top:0;" class="only-pro"><a href="https://thememattic.com/minimal-blocks-pro-demos/" target="_blank" >';
					echo esc_html ('Minimal Blocks Pro Live Preview', 'minimal-blocks');
					echo '</a></td></tr>';
        ...
        'activating_string'        => __( 'Activating', 'minimal-blocks' )
        )
    );
