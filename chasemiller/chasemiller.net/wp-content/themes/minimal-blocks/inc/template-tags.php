	...
	function minimal_blocks_posted_on() {
		...
		if(is_single()){
            ...
            if ( $categories_list ) {
                ...
                printf( '<span class="cat-links"><span class="thememattic-icon ion-ios-folder-outline"></span>' . esc_html__( ' %1$s', 'minimal-blocks' ) . '</span>', $categories_list ); // WPCS: XSS OK.
            }
        }
	}
endif;
    ...
    function minimal_blocks_entry_category()
    {
        ...
        if ('post' === get_post_type()) {
            ...
            if ($categories_list && minimal_blocks_categorized_blog()) {
                printf(("<span class='entry-meta meta-categories'>".esc_html__('%1$s', 'minimal-blocks')."</span>"), $categories_list);
            }
        }
    }
endif;
...
if (!function_exists('minimal_blocks_posted_comment')) :
    function minimal_blocks_posted_comment()
    {
        if (!is_single() && !post_password_required() && (comments_open() || get_comments_number())) {
            ...
            comments_popup_link( esc_html__( ' ', 'minimal-blocks' ), esc_html__( '1 Comment', 'minimal-blocks' ), esc_html__( '% Comments', 'minimal-blocks' ) );
            echo '</span>';
        } 
    }
endif;
	...
	function minimal_blocks_entry_footer() {
		...
		if ( 'post' === get_post_type() ) {
			...
			if ( $tags_list ) {
				...
				printf( '<span class="tags-links"><span class="thememattic-icon ion-ios-pricetags-outline"></span>' . esc_html__( '%1$s', 'minimal-blocks' ) . '</span>', $tags_list ); // WPCS: XSS OK.
			}
		}
